...
const FRASES_CARGA_CORTAS = "👗 Eligiendo tu talla perfecta...,🛍️ Preparando las últimas referencias...,✨ Acomodando el catálogo...,💃 Casi listo para verte hermosa...,🪡 Revisando las costuras...,👠 Combinando el look...,🎀 Envolviendo con cariño...,💕 Seleccionando tus favoritos...,🌸 Colgando prendas nuevas...,👜 Organizando los accesorios...,💳 Alistando el checkout...,📦 Empacando con amor...,🩷 Puliendo los detalles...,👑 Preparando algo especial para ti...,🫶 Con todo el cariño para ti...,🌺 Casi lista tu experiencia...,🪄 Haciendo magia con tu talla...,💐 Arreglando el escaparate...,🧵 Cosiendo los últimos detalles...,👛 Escogiendo el bolso ideal...,🎁 Envolviendo tu sorpresa...,💫 Brillando para ti...,🛒 Llenando el carrito de opciones...,📏 Confirmando las tallas disponibles...,🌷 Refrescando el catálogo...,👒 Probando accesorios...,🦋 Transformando tu clóset...,🧣 Ajustando los complementos...,💅 Puliendo cada detalle...".split(",");
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "linear-gradient(160deg, #FFF5F7 0%, #FCE4EC 50%, #F8BBD0 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 9999, fontFamily: "'DM Sans', sans-serif", overflow: "hidden" }}>
      <style>{`
        ...
        @keyframes arena-arriba { from { transform: scaleY(1); } to { transform: scaleY(0.1724); } }
        @keyframes arena-abajo { from { transform: scaleY(0.1724); } to { transform: scaleY(1); } }
        @keyframes arena-hilo { 0%, 6% { opacity: 0; transform: scaleY(0.333); } 8% { opacity: 0.7; } 92% { opacity: 0.7; transform: scaleY(1); } 94%, 100% { opacity: 0; transform: scaleY(1); } }
        .arena-arriba-rect { transform-origin: 40px 70px; animation: arena-arriba 5s linear infinite; }
        .arena-abajo-rect { transform-origin: 40px 118px; animation: arena-abajo 5s linear infinite; }
        .arena-hilo-linea { transform-origin: 40px 60px; animation: arena-hilo 5s linear infinite; }
      `}</style>
      ...
      <div style={{ marginBottom: 32, filter: "drop-shadow(0 8px 20px rgba(194,24,91,0.25))", animation: "flotar 4s ease-in-out infinite" }}>
        <svg width={80} height={120} viewBox="0 0 80 120" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <g clipPath="url(#clipArriba)"><rect className="arena-arriba-rect" x="14" y="12" width="52" height="58" fill="url(#gradArena)" /></g>
          <g clipPath="url(#clipAbajo)"><rect className="arena-abajo-rect" x="14" y="60" width="52" height="58" fill="url(#gradArena)" /></g>
          <line className="arena-hilo-linea" x1="40" y1="60" x2="40" y2="75" stroke="#C2185B" strokeWidth="1.5" strokeLinecap="round" />
  ...
  return (
    <div style={{ position: "sticky", top: 0, zIndex: 600, boxShadow: conSombra ? "0 2px 12px rgba(0,0,0,0.08)" : "none", transition: "box-shadow 0.3s ease" }}>
      <div className="topbar">
        <div className="topbar__marquee" style={{ display: "flex", animation: "ticker 45s linear infinite", width: "max-content" }}>
          {[0, 1, 2, 3].map((i) => (
